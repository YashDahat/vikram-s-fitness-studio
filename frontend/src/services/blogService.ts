...
import type { BlogPostDto, WorkoutLogDto } from '@/types/blog';
...
export const logWorkout = async (request: WorkoutLogDto): Promise<WorkoutLogDto> => {
  const response = await apiClient.post<WorkoutLogDto>('/api/v1/progress/workouts', request);
  return response.data;
};

export const getWorkoutLogs = async (): Promise<WorkoutLogDto[]> => {
  const response = await apiClient.get<WorkoutLogDto[]>('/api/v1/progress/workouts');
  return response.data;
};
